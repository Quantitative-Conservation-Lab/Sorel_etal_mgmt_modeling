...
sim_out <- foreach::foreach(RS =1:n_rest, .combine = 'rbind', .packages = c("tidyverse","TMB")) %dopar% {
  ...
  for(BS in 1:7){
    ...
    ave_ad_LH_props<-apply(sim_LH_props,1,mean,na.rm=TRUE
    )
    ...
    # ave_ad_LH_props<-apply(sim_LH_props,1,mean,na.rm=TRUE
    # )
    # as.tibble(ave_ad_LH_props |> t()) |> `colnames<-` (c("fry","sum","fal","Spr")) |> mutate(BS=BS_scenario,multiplier=multiplier,Strategy=Strategy)
    # 
    # out<-bind_rows(out,as.tibble(ave_ad_LH_props |> t()) |> `colnames<-` (c("fry","sum","fal","Spr")) |> mutate(BS=BS_scenario,multiplier=multiplier,Strategy=Strategy))
...
sim_out |> pivot_longer(fry:Spr) |>  ggplot(aes(x=multiplier,y=value,fill=name))+geom_col()+facet_wrap(~Str)

rm(list=ls())
gc()

load(file=here("results","sim_out_6_06_25.rda"))

# sim_out |> filter(Strategy%in%c("All restored","Baseline")) |> 
#   pivot_longer(c(fry:Spr),names_to="LHP",values_to = "prop") |> 
#   mutate(LHP=fct_relevel(LHP,c("fry","sum","fal","Spr"))) |> 
#   ggplot(aes(x=multiplier,y=prop,fill=LHP))+geom_col()
...
top<-new_summarized_sims|> filter(Hatchery==1,var=="abund",stream=="Total",Strategy%in%c("All restored","All natal only","Downstream only")) |> 
  ...
  geom_text(data = facet_labels[1:3,], aes(x = 1.1, y = 2.5, label = label), inherit.aes = FALSE,size=6)+
  theme(
...
bottom<-new_summarized_sims|> filter(Hatchery==1,var=="abund",stream=="Total",!Strategy%in%c("All restored","All natal only","Downstream only")) |> 
  ...
  geom_text(data = facet_labels[4:6,], aes(x = 1.1, y = 1.25, label = label), inherit.aes = FALSE,size=6)+scale_x_continuous(breaks=c(1,1.2,1.4))
...
top4<-new_summarized_sims|> filter(Strategy=="All restored",var=="abund",stream=="Total",multiplier<1.3) |> mutate(Hab_mult=as.factor(multiplier)) |> ggplot(aes(x=Hatchery))+geom_linerange(aes(ymax=`4`/1000,ymin=`2`/1000),lwd=1.05)+
  ...
  geom_text(data = facet_labels2[1:3,], aes(x = 0.1, y = 1.25, label = label), inherit.aes = FALSE,size=6)





bottom4<-  new_summarized_sims|> filter(Strategy=="All restored",var=="abund",stream=="Total",multiplier>=1.3) |> mutate(Hab_mult=as.factor(multiplier)) |> ggplot(aes(x=Hatchery))+geom_linerange(aes(ymax=`4`/1000,ymin=`2`/1000),lwd=1.05)+
  geom_point(aes(y=`3`/1000),size=2)+facet_wrap(~Hab_mult)+
  # geom_line(aes(y=`3`/1000))+
  ylab("")+xlab("Hatchery size proportion")+theme(text = element_text(size = 16))+ 
  geom_text(data = facet_labels2[4:6,], aes(x = 0.1, y = 2.35, label = label), inherit.aes = FALSE,size=6)
